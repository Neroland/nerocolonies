#!/usr/bin/env python3
"""
Generate the NEROCOLONIES mod logo (square), in the shared Neroland family style
...
the colony/settlement mod, so the core-prism is ringed by little ORANGE-lit settlement houses Renders supersampled, then downsamples.

Outputs:
  art/logo/nerocolonies_logo.png       (1024x1024 master)
  art/logo/nerocolonies_logo_400.png   (CurseForge/Modrinth-ready)
  common/src/main/resources/nerocolonies_logo.png  (256x256 in-game mods-list icon)
"""
SEED = 43
MOD = 'nerocolonies'
NAME = 'NEROCOLONIES'
ACCENT = (228, 140, 66)
ACCENT_BRIGHT = (255, 190, 128)
BRIGHT = (255, 238, 220)
NEBULA = (116, 62, 20)
STAR_TINT = (255, 216, 176)
NAME_GLOW = (228, 140, 66, 255)
TAG_COL = (234, 194, 158, 255)
...
TAG = 'S E T T L E   ·   B U I L D   ·   G R O W'

def emblem_frame(base, cx, cy, rad):
    ring = rad * 1.62
    gl = Image.new("RGBA", (R, R), (0, 0, 0, 0)); gd = ImageDraw.Draw(gl)
    gd.ellipse([cx - ring, cy - ring, cx + ring, cy + ring], outline=ACCENT + (160,), width=SS * 2)
    ...
    for i in range(8):
        ...
        hx, hy = cx + math.cos(a) * ring, cy + math.sin(a) * ring
        # little house: body + roof
        hd.rectangle([hx - hw, hy - hw * 0.2, hx + hw, hy + hw], fill=STEEL_DK + (255,),
                     outline=ACCENT + (235,), width=SS)
        hd.polygon([(hx - hw * 1.15, hy - hw * 0.2), (hx + hw * 1.15, hy - hw * 0.2),
                    (hx, hy - hw * 1.1)], fill=ACCENT + (255,))
